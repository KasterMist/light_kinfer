...
def _apply_aliases(
    ...
    返回一个浅拷贝的字典，其中别名键被重命名为目标键。
    """
    out: dict[str, Any] = dict(raw)  # 创建原始字典的浅拷贝
    for old, new in aliases.items():  # 遍历别名映射
        if old in out:  # 如果旧键存在
            out[new] = out.pop(old)  # 将旧键的值赋给新键，并删除旧键
    return out
...
def _filter_fields(data: Mapping[str, Any], cls: Type) -> dict[str, Any]:
    ...
    丢弃在 *cls* 数据类字段中未声明的键。
    """
    valid = {f.name for f in fields(cls)}  # 获取数据类中所有有效字段名
    return {k: v for k, v in data.items() if k in valid}  # 只保留有效字段


# ----------------------------------------------------------------------------- #
#                            BaseConfig definition 基础配置定义                   #
# ----------------------------------------------------------------------------- #
@dataclass
class BaseConfig:
    """Minimal base class providing *from_dict* utility.
    提供 *from_dict* 工具的最小基类。
    """

    # subclasses may override 子类可以重写此字段
    ...
    def from_dict(cls: Type[T], data: Mapping[str, Any]) -> T:
        ...
        # only keep valid fields 只保留有效字段
...
class LlamaConfig(BaseConfig):
    ...
    def __post_init__(self) -> None:
        """初始化后的参数验证和默认值设置"""
        # 如果未指定KV头数量，则设为与注意力头数量相同
        if self.num_kv_heads is None:
            self.num_kv_heads = self.num_heads
        # 如果未指定中间层大小，则设为隐藏层大小的4倍
        if self.intermediate_size is None:
            self.intermediate_size = self.hidden_size * 4

        # 如果未指定头维度，则根据隐藏层大小和头数量计算
